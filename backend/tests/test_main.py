...
async def test_get_products_search(client: AsyncClient, test_product: Product):
    ...
    resp = await client.get("/api/v1/products?category=Untitled")
    assert resp.status_code == status.HTTP_404_NOT_FOUND

    resp = await client.get("/api/v1/products?category=Test")
    assert resp.status_code == status.HTTP_200_OK

    resp = await client.get("/api/v1/products?name=Untitled")
    assert resp.status_code == status.HTTP_404_NOT_FOUND

    resp = await client.get("/api/v1/products?name=Test")
    assert resp.status_code == status.HTTP_200_OK
